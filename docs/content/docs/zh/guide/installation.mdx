...
bash <(curl -Ls https://raw.githubusercontent.com/ifwqu/3x-ui/master/install.sh)
```

在命令后附加标签即可安装**指定版本**：

```bash title="pinned version"
bash <(curl -Ls https://raw.githubusercontent.com/ifwqu/3x-ui/master/install.sh) v3.4.0
```
...
bash <(curl -Ls https://raw.githubusercontent.com/ifwqu/3x-ui/master/install.sh) dev-latest
```
...
高级用户可从[发布页面](https://github.com/ifwqu/3x-ui/releases)
下载适用于你架构的发布归档包，解压后
...
```bash
XUI_NONINTERACTIVE=1 bash <(curl -Ls https://raw.githubusercontent.com/ifwqu/3x-ui/master/install.sh)
```

仓库的 [`deploy/`](https://github.com/ifwqu/3x-ui/tree/main/deploy)
...
<Cards>
  <Card
    title="一键搭建节点"
    href="/docs/guide/oneclick-setup"
    description="使用一键脚本快速创建 VLESS+REALITY、TROJAN+TLS 等节点。"
  />
  <Card
    title="首次登录"
